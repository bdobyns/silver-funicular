...
In a separate companion document we show the speciic steps we actually used to rescue a specific Centos4.6 application which used some nonstandard RPMS from a VMware image. 
...
2. Once moved you have time and leisure to decide how to update the application to be on a more modern software stack

# LINUX DISTRO SPECIFIC ISSUES

* Note that Debian is much more harsh about taking down public repositories of ancient distributions.  There is no way to get a copy of Debian Potato anymore.
...
* We assume you have ssh access to the failing instance, and sudo or root credentials.  
...
*  For An instance you can stop and tinker with (like a vmware image).   
   ...
   * tar it up.  note that we don't need to exclude /proc and /dev this time because they're not present (mounted) in this case  
...
* Inside a docker container, you don't really get to run init and process all the /etc/init.d scripts the way you do in a regular linux instance.   
   ...
   * now, you need to examine the init scripts for that runlevel, typically in `/etc/rc3.d`
      ...
      * most things that normally run in a linux box are *totally unnecessary* in a docker container.  
      For example, in a typical Centos4 box, none of these are necessary to run in a Docker container: 
...
```
   ...
   * The short list of things you MUST run probably really is short.   
     * For a PHP app it may be just `mysqld` and `httpd`.   
     * For a complicated app with some stuff in java and some stuff served by apache, it might be `mysqld` and `httpd` plus `tomcat`
